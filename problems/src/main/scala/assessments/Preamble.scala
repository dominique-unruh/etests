...
object Preamble {
  ...
  def textinput(correct: Seq[String], wrong: Seq[String], partiallyCorrect: Map[String,Points] = Map.empty, points: Points)(using context: Context): TextInput =
    TextInput(name=context.name, correct = correct, wrong = wrong, points = points, partiallyCorrect = partiallyCorrect)

  def grader(python: String = null, points: Points, check: Map[ElementName, String] => Points|Boolean = null)(using context: Context): Grader =
    Grader(name=context.name, points=points, python=python, check=check)
}
