...
class DockerTest extends AnyFunSuiteLike {
  test ("cp workdir") {
    val result = Docker.runInDocker(image = "docker.io/library/archlinux:latest", command = "cp in.txt out.txt".split(' '),
      files=Map("in.txt" -> "hello"), requestedOutputs = Seq("out.txt"),
      shortDescription = "cp workdir").awaitResult()
  ...
  test("Concurrency") {
    ...
    val futures = for (i <- 1 to 10) yield
      Docker.runInDocker(image = "docker.io/library/archlinux:latest", command = Seq("sleep", (i+5).toString),
        files = Map("in.txt" -> random.toString), requestedOutputs = Seq.empty,
        shortDescription = "sleep")
